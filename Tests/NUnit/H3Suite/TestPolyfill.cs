...
namespace TestSuite
{
    ...
    public class TestPolyfill
    {
        ...
        {
            new GeoCoord(0.6599669176550, -2.1364398519396),
        ...
        private static void fillIndex_assertions(H3Index h)
        {
            ...
            for (int nextRes = currentRes; nextRes <= currentRes + 1; nextRes++)
            {
                ...
                polyfillOut = polyfillOut.Where(p => p != Constants.H3_NULL).ToList();
                polyfillOut.Sort();

                children = children.Where(p => p != Constants.H3_NULL).ToList();
                children.Sort();
